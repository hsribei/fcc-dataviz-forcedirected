...
var spin_js_1 = __webpack_require__(472);
var image = __webpack_require__(473);
var url = "https://raw.githubusercontent.com/DealPete/forceDirected/master/countries.json";
var spinner;
document.addEventListener("DOMContentLoaded", function () {
    var content = d3.select("#content");
    console.log(content.node());
    spinner = new spin_js_1.Spinner({ color: "white" }).spin(content.node());
});
d3.json(url, function (error, graph) {
    ...
    if (error)
        throw error;
    // Give nodes ids
    graph.nodes.forEach(function (node, i) { return (node.id = i); });
    var nodeRadius = 8;
    var nodeDiameter = nodeRadius * 2;
    var boundingRadius = 480;
    var width = boundingRadius * 2, height = boundingRadius * 2;
    var simulation = d3
        .forceSimulation()
        .force("link", d3.forceLink().id(function (d) { return d.id; }))
        .force("charge", d3.forceManyBody().strength(-150))
        .force("center", d3.forceCenter(boundingRadius, boundingRadius))
        .force("radial", d3.forceRadial(0.0001, boundingRadius, boundingRadius).strength(0.1));
    var svg = d3
        .select("svg#d3")
    ...
    function ticked() {
        link
            .attr("x1", function (d) {
            return Math.max(nodeRadius, Math.min(width - nodeRadius, d.source.x));
        })
            .attr("y1", function (d) {
            return Math.max(nodeRadius, Math.min(height - nodeRadius, d.source.y));
        })
            .attr("x2", function (d) {
            return Math.max(nodeRadius, Math.min(width - nodeRadius, d.target.x));
        })
            .attr("y2", function (d) {
            return Math.max(nodeRadius, Math.min(height - nodeRadius, d.target.y));
        });
        node
            .attr("x", function (d) {
            return Math.max(0, Math.min(width - nodeDiameter, d.x - nodeRadius));
        })
            .attr("y", function (d) {
            return Math.max(0, Math.min(height - nodeDiameter, d.y - nodeRadius));
        });
    }
...
module.exports = __webpack_require__.p + "0b8915998d4be63d4e9c88d3d917300f.png";
...
module.exports = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAAAAACH5BAEAAAAALAAAAAABAAEAAAgFAAEACAgAOw=="
